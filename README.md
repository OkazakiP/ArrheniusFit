...
左辺の分子は樹脂の残存量を表すので，結局は劣化する成分だけに着目すれば[定数項がない場合の方法](#アレニウスプロットの定式化)に帰結します．

### 複数の成分が劣化する場合

[上記](#特性と濃度の関係に定数項がある場合)の通り変化する成分だけに注目すれば[特性が濃度に比例する場合](#アレニウスプロットの定式化)の議論が使えます．複数成分を含む場合もそれぞれが単純に1次反応で劣化する場合はそれぞれの和で特性は予測できます．あえて式で書くと次の通りです．
...
この通り冪乗に比例する場合も $nA = A'$ の置換によって[1次の場合](#アレニウスプロットの定式化)に帰結されます．

### 特性が濃度の対数に比例する場合

吸光度やpHは常用対数で表される特性です．特にpHはプロトン濃度を用いて $\text{pH} = -log[\text{H}^+]$ と定義されます．[劣化度を固定しない場合](#劣化量を固定しない方法)の式において $m(t, T) = [\text{H}^+](t, T)$ と濃度そのものを代入してみます．

$$
-ln \left( -ln \frac{[\text{H}^+](t, T)}{[\text{H}^+](0, T)} \right) = \frac{E_a}{RT} - ln (t) - ln (A)
$$

$$
-ln \left( -ln \frac{10^{-\text{pH}(t, T)}}{10^{-\text{pH}(0, T)}} \right) = \frac{E_a}{RT} - ln (t) - ln (A)
$$

$$
-ln \left( ln(10) \times (\text{pH}(t, T) - \text{pH}(0, T)) \right) = \frac{E_a}{RT} - ln (t) - ln (A)
$$

$$
-ln \left( \text{pH}(t, T) - \text{pH}(0, T) \right) = \frac{E_a}{RT} - ln (t) - ln (A) + ln(ln(10))
$$

通常は対数を2回取る必要がありますが，対数で表される特性は1回だけかつ比の代わりに差を用いることになります．なおpHを与えるプロトンは酸塩基平衡に支配されるため，材料の劣化反応だけで単純には決まりません．しかしながら強酸が生じる場合や，塩基性条件下で酸が生じる場合などプロトンの完全解離が仮定できる場合にはこの式が適用できます．
...
$\alpha$ は積分定数です．右辺では $S, C$ の初期値 $S_0, C_0$ を使って表現しました．これを元の反応速度式に代入します．
...
ここにアレニウスの式を代入しても，[1次反応の時](#アレニウスプロットの定式化) のような $t, T (1/T)$ の線形の式は得られません．
...
(4)式が $S$ の初期値を $S_0$ として $S = S_0 e^{-k_1t}$ と解かれるのは[上記](#アレニウスプロットの定式化)の通りです．これと(6)式を(5)式に代入して整理します．
...
## 非線形関数の回帰
結局，特性が1次反応で変化する化学種の濃度の冪乗以外で表される場合，簡単な線形の式で寿命を予測することはできなさそうです．前置きが長くなりましたが，このため材料寿命の予測には非線形回帰が必要になります．ただし劣化反応の次数がいくつだとか，特性を材料濃度で表すモデルだとかは扱う材料によって変更しなければなりません．そこで，本プログラムでは反応モデルと特性モデルを予め準備することとします．